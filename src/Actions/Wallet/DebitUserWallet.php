...
class DebitUserWallet extends Action
{
    ...
    private function setUserWallet()
    {
        ...
        if (array_key_exists('user_id', $this->validatedData) && $this->validatedData['user_id'] && auth()->user()->role=='admin') {
    ...
    private function debitWallet()
    {
        ...
        $this->wallet->update([
            'balance' => (float)$current_balance - abs((float)$this->validatedData['amount']),
            'previous_balance' => $current_balance
        ]);
        return $this->sendSuccess($this->wallet->refresh()->load('user'), 'wallet debited successfully');
    }

    private function setType()
    {
        $this->validatedData['type'] = 'debit';
        return $this;
    }
    ...
    private function setDescription()
    {
        if (!array_key_exists('description', $this->validatedData)) {
            $this->validatedData['description'] = "Debit from user wallet";
        }
        return $this;
    }
    ...
    private function setCategory()
    {
        if (!array_key_exists('category', $this->validatedData)) {
            $this->validatedData['category'] = "WALLET_WITHDRAWAL";
        }
        return $this;
    }
